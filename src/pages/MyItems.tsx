...
import { Clock, MapPin, Package, MessageSquare, CheckCircle, XCircle, Search } from "lucide-react";
        ...
        async (payload) => {
          ...
          if (matches.length > 0) {
            ...
            toast({
              ...
              description: `New ${newItem.type} item might match your "${bestMatch.item.title}" (${Math.round(bestMatch.score)}% match)`,
              duration: 8000,
              action: (
                <Button size="sm" onClick={() => navigate(`/item/${newItem.id}`)}>
                  View Item
                </Button>
              ),
            });
